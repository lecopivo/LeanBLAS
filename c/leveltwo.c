 ...
 * Computes a matrix-vector product using a general matrix.
  ...
  */
LEAN_EXPORT lean_obj_res leanblas_cblas_dgemv(
    const uint8_t order, const uint8_t transA, const size_t M, const size_t N,
    const double alpha, const b_lean_obj_arg A, const size_t offA,
    const size_t lda, const b_lean_obj_arg X, const size_t offX,
    const size_t incX, const double beta, lean_obj_arg Y, const size_t offY,
    const size_t incY) {
  ...
  cblas_dgemv(leanblas_cblas_order(order), leanblas_cblas_transpose(transA),
              ...
              lean_float_array_cptr(X) + offX, (int)incX, beta,
...
/** dgbmv
  ...
  * Computes a matrix-vector product using a general band matrix.
    ...
    * @return Y with the matrix-vector product added to it
    */
LEAN_EXPORT lean_obj_res leanblas_cblas_dgbmv(
    const uint8_t order, const uint8_t transA, const size_t M, const size_t N,
  ...
  cblas_dgbmv(leanblas_cblas_order(order), leanblas_cblas_transpose(transA),
              (int)M, (int)N, (int)KL, (int)KU, alpha,
 ...
 */
LEAN_EXPORT lean_obj_res leanblas_cblas_dtrmv(
    const uint8_t order, const uint8_t uplo, const uint8_t transA,
    const uint8_t diag, const size_t N, const b_lean_obj_arg A,
    const size_t offA, const size_t lda, lean_obj_arg X, const size_t offX,
    const size_t incX) {
  ensure_exclusive_byte_array(&X);

  cblas_dtrmv(leanblas_cblas_order(order), leanblas_cblas_uplo(uplo),
              leanblas_cblas_transpose(transA), leanblas_cblas_diag(diag),
              (int)N, lean_float_array_cptr(A) + offA, (int)lda,
 ...
 * @return X with the matrix-vector product added to it
 */
LEAN_EXPORT lean_obj_res leanblas_cblas_dtbmv(
    const uint8_t order, const uint8_t uplo, const uint8_t transA,
    const uint8_t diag, const size_t N, const size_t K, const b_lean_obj_arg A,
    const size_t offA, const size_t lda, lean_obj_arg X, const size_t offX,
    const size_t incX) {
  ensure_exclusive_byte_array(&X);

  cblas_dtbmv(leanblas_cblas_order(order), leanblas_cblas_uplo(uplo),
              leanblas_cblas_transpose(transA), leanblas_cblas_diag(diag),
              (int)N, (int)K, lean_float_array_cptr(A) + offA, (int)lda,
              lean_float_array_cptr(X) + offX, (int)incX);
 ...
 * @return X with the matrix-vector product added to it
 */
LEAN_EXPORT lean_obj_res leanblas_cblas_dtpmv(
    const uint8_t order, const uint8_t uplo, const uint8_t transA,
    const uint8_t diag, const size_t N, const b_lean_obj_arg A,
    const size_t offA, lean_obj_arg X, const size_t offX, const size_t incX) {
  ensure_exclusive_byte_array(&X);

  cblas_dtpmv(leanblas_cblas_order(order), leanblas_cblas_uplo(uplo),
              leanblas_cblas_transpose(transA), leanblas_cblas_diag(diag),
              (int)N, lean_float_array_cptr(A) + offA,
              lean_float_array_cptr(X) + offX, (int)incX);
 ...
 */
LEAN_EXPORT lean_obj_res leanblas_cblas_dtrsv(
    const uint8_t order, const uint8_t uplo, const uint8_t transA,
    const uint8_t diag, const size_t N, const b_lean_obj_arg A,
    const size_t offA, const size_t lda, lean_obj_arg X, const size_t offX,
    const size_t incX) {
  ...
  cblas_dtrsv(leanblas_cblas_order(order), leanblas_cblas_uplo(uplo),
              leanblas_cblas_transpose(transA), leanblas_cblas_diag(diag),
              (int)N, lean_float_array_cptr(A) + offA, (int)lda,
              lean_float_array_cptr(X) + offX, (int)incX);
 ...
 * @return X with the solution to the system of linear equations
 */
LEAN_EXPORT lean_obj_res leanblas_cblas_dtbsv(
    const uint8_t order, const uint8_t uplo, const uint8_t transA,
    const uint8_t diag, const size_t N, const size_t K, const b_lean_obj_arg A,
    const size_t offA, const size_t lda, lean_obj_arg X, const size_t offX,
    const size_t incX) {
  ...
  cblas_dtbsv(leanblas_cblas_order(order), leanblas_cblas_uplo(uplo),
              leanblas_cblas_transpose(transA), leanblas_cblas_diag(diag),
              (int)N, (int)K, lean_float_array_cptr(A) + offA, (int)lda,
              lean_float_array_cptr(X) + offX, (int)incX);
 ...
 * @return X with the solution to the system of linear equations
 */
LEAN_EXPORT lean_obj_res leanblas_cblas_dtpsv(
    const uint8_t order, const uint8_t uplo, const uint8_t transA,
    const uint8_t diag, const size_t N, const b_lean_obj_arg A,
    const size_t offA, lean_obj_arg X, const size_t offX, const size_t incX) {
  ensure_exclusive_byte_array(&X);

  cblas_dtpsv(leanblas_cblas_order(order), leanblas_cblas_uplo(uplo),
              leanblas_cblas_transpose(transA), leanblas_cblas_diag(diag),
              (int)N, lean_float_array_cptr(A) + offA,
              lean_float_array_cptr(X) + offX, (int)incX);
 ...
 */
LEAN_EXPORT lean_obj_res leanblas_cblas_dger(
    const uint8_t order, const size_t M, const size_t N, const double alpha,
    const b_lean_obj_arg X, const size_t offX, const size_t incX,
...
/** syr
 *
 * Computes the outer product of a vector with itself and adds it to a symmetric
 * matrix.
 *
 * @param order Row or column major
 * @param uplo Upper or lower triangular
 ...
 * @return A with the outer product added to it
 */
LEAN_EXPORT lean_obj_res leanblas_cblas_dsyr(
    const uint8_t order, const uint8_t uplo, const size_t N, const double alpha,
    const b_lean_obj_arg X, const size_t offX, const size_t incX,
    lean_obj_arg A, const size_t offA, const size_t lda) {
  ensure_exclusive_byte_array(&A);

  cblas_dsyr(leanblas_cblas_order(order), leanblas_cblas_uplo(uplo), (int)N,
             alpha, lean_float_array_cptr(X) + offX, (int)incX,
             lean_float_array_cptr(A) + offA, (int)lda);
 ...
 * @return A with the outer product added to it
 */
LEAN_EXPORT lean_obj_res leanblas_cblas_dsyr2(
    const uint8_t order, const uint8_t uplo, const size_t N, const double alpha,
    const b_lean_obj_arg X, const size_t offX, const size_t incX,
    const b_lean_obj_arg Y, const size_t offY, const size_t incY,
    lean_obj_arg A, const size_t offA, const size_t lda) {
  ...
  cblas_dsyr2(leanblas_cblas_order(order), leanblas_cblas_uplo(uplo), (int)N,
              alpha, lean_float_array_cptr(X) + offX, (int)incX,
              lean_float_array_cptr(Y) + offY, (int)incY,
              lean_float_array_cptr(A) + offA, (int)lda);
